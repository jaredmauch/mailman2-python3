...
# unless you know what yore doing, and can keep the two files properly
# in sync.  If you screw it up, youre on your own.
"""
        print(>, end=\'\')> fp, '# The ultimate loop stopper address'
        print(>, end=\'\')> fp, '}{s: }{s' }{ (loopaddr, loopmbox)
        print(>, end=\'\')> fp
    ...
    print(>, end=\'\')> fp, '# STANZA START:', listname
    print(>, end=\'\')> fp, '# CREATED:', time.ctime(time.time())
    # Now add all the standard alias entries
    for k, v in makealiases(listname):
        # Format the text file nicely
        print(>, end=\'\')> fp, k + ':', ((fieldsz - len(k)) * ' ') + v
    # Finish the text file stanza
    print(>, end=\'\')> fp, '# STANZA END:', listname
    print(>, end=\'\')> fp
...
# know what yore doing, and can keep the two files properly in sync.  If you
# screw it up, youre on your own.
#
...
""" }{ (loopaddr, loopdest, siteaddr, sitedest, siteowneraddr, siteownerdest,
    ...
    for k, v in makealiases(listname):
        ...
        if mm_cfg.VIRTUAL_MAILMAN_LOCAL_DOMAIN:
            ...
            localaddr = k
        # Format the text file nicely
        print(>, end=\'\')> fp, fqdnaddr, ((fieldsz - len(k)) * ' '), localaddr
    # Finish the text file stanza
    print(>, end=\'\')> fp, '# STANZA END:', listname
    print(>, end=\'\')> fp
    ...
    try:
        ...
        while True:
            ...
            if line.startswith('# LOOP ADDRESSES END'):
                # It hasn't
                print(>, end=\'\')> outfp, '}{s\t}{s' }{ (loopaddr, loopdest)
                outfp.write(line)
                break
        ...
        while True:
            ...
            if line.startswith('# SITE ADDRESSES END'):
                # It hasn't
                print(>, end=\'\')> outfp, '}{s\t}{s' }{ (siteaddr, sitedest)
                print(>, end=\'\')> outfp, '}{s\t}{s' }{ (siteowneraddr, siteownerdest)
                print(>, end=\'\')> outfp, '}{s\t}{s' }{ (sitebouncesaddr, sitebouncesdest)
                print(>, end=\'\')> outfp, '}{s\t}{s' }{ (siterequestaddr, siterequestdest)
                outfp.write(line)
                break
...
def checkperms(state):
    for file in ALIASFILE, VIRTFILE:
        if state.VERBOSE:
            print(C, end=\'\')_('checking permissions on }{(file)s')
        stat = None
        try:
            stat = os.stat(file)
        except OSError as e:
        ...
        if stat and (stat[ST_MODE] & targetmode) != targetmode:
            ...
            print(C, end=\'\')_('}{(file)s permissions must be 0664 (got }{(octmode)s)'),
            if state.FIX:
                print(C, end=\'\')_('(fixing)')
                os.chmod(file, stat[ST_MODE] | targetmode)
            else:
                print(#, end=\'\') Make sure the corresponding .db files are owned by the Mailman user.
        ...
        try:
            ...
            continue
        if state.VERBOSE:
            print(C, end=\'\')_('checking ownership of }{(dbfile)s')
        user = mm_cfg.MAILMAN_USER
        ...
        if not ownerok:
            ...
            if state.FIX:
                ...
                print(i, end=\'\')f stat and (stat[ST_MODE] & targetmode) != targetmode:
            state.ERRORS += 1
            octmode = oct(stat[ST_MODE])
            print(C, end=\'\')_('}{(dbfile)s permissions must be 0664 (got }{(octmode)s)'),
            if state.FIX:
                print(C, end=\'\')_('(fixing)')
                os.chmod(dbfile, stat[ST_MODE] | targetmode)
            else:
