...
""", file=fp)
        print('# The ultimate loop stopper address', file=fp)
        print('%s: %s' % (loopaddr, loopmbox), file=fp)
        print(file=fp)
    ...
    print('# STANZA START:', listname, file=fp)
    print('# CREATED:', time.ctime(time.time()), file=fp)
    # Now add all the standard alias entries
    for k, v in makealiases(listname):
        # Format the text file nicely
        print(k + ':', ((fieldsz - len(k)) * ' ') + v, file=fp)
    # Finish the text file stanza
    print('# STANZA END:', listname, file=fp)
    print(file=fp)
...
""" % (loopaddr, loopdest, siteaddr, sitedest, siteowneraddr, siteownerdest,
       sitebouncesaddr, sitebouncesdest, siterequestaddr, siterequestdest), file=fp)
    # The text file entries get a little extra info
    print('# STANZA START:', listname, file=fp)
    print('# CREATED:', time.ctime(time.time()), file=fp)
    # Now add all the standard alias entries
    for k, v in makealiases(listname):
        ...
        if mm_cfg.VIRTUAL_MAILMAN_LOCAL_DOMAIN:
            ...
            localaddr = k
        # Format the text file nicely
        print(fqdnaddr, ((fieldsz - len(k)) * ' '), localaddr, file=fp)
    # Finish the text file stanza
    print('# STANZA END:', listname, file=fp)
    print(file=fp)
